...
const Player = {
    ...
    init() {
        ...
        if (videoPlayer) {
            ...
            // Error listener যোগ করা
            // এখানে .bind(this) ব্যবহার করে রেফারেন্স তৈরি করা হয়েছে, যা removeEventListener-এও ব্যবহার করা যাবে
            this.handleErrorBound = this.handlePlaybackError.bind(this);
        }
        
        const switchBtn = document.getElementById('player-switch-btn');
        if (switchBtn) {
            // Switch বাটন ক্লিক হ্যান্ডলার যোগ করা (যাতে HTML এ onclick না থাকলেও কাজ করে)
    ...
    updatePlayerUI() {
        ...
        // Native Player এর ডিসপ্লে ঠিক করা
    ...
    // handlePlaybackError ফাংশন পরিবর্তন: নোটিফিকেশন সরিয়ে অটো-সুইচ লজিক
    handlePlaybackError(event) {
        if (this.currentPlayerMode === 'native' && !event.target.seeking) {
            // নোটিফিকেশন ছাড়া সরাসরি সুইচ করা
