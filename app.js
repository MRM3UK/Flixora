...
const CONFIG = {
    ...
    STORAGE_KEYS: {
        ...
        // PLAYER_MODE এখন Settings-এর অংশ হবে
    }
};
...
const Library = {
    render() {
        ...
        document.getElementById('library-playlists-grid').innerHTML = playlistsHtml; // ID ঠিক করা হয়েছে
        ...
        document.getElementById('library-favorites-grid').innerHTML = favoritesHtml; // ID ঠিক করা হয়েছে
        ...
        document.getElementById('library-history-grid').innerHTML = historyHtml; // ID ঠিক করা হয়েছে
    },
...
// ===== Settings (Updated to handle Player Mode) =====
const Settings = {
    defaultSettings: {
        autoplay: true,
        quality: 'auto',
    ...
    save() {
        const settings = {
            ...
            playerMode: document.getElementById('setting-player').value // New Player Mode
        };
        
        Utils.setStorage(CONFIG.STORAGE_KEYS.SETTINGS, settings);
        
        // Player Mode পরিবর্তন হলে Player-কে আপডেট করা
        if (Player.currentPlayerMode !== settings.playerMode) {
            Player.currentPlayerMode = settings.playerMode;
            Player.destroyAllPlayers(); // পুরোনো প্লেয়ার বন্ধ
            Player.updatePlayerVisibility(); // নতুন প্লেয়ার কন্টেইনার দেখানো
...
// ===== Player (Core) - **MAJOR CHANGES HERE** =====
const Player = {
    currentPlaylist: [],
    currentIndex: -1,
    hlsInstance: null,
    playerjsInstance: null,
    clapprInstance: null, // New Clappr instance
    ...
    init() {
        ...
        // Remove old switch button logic as settings handle player selection now
    ...
    destroyAllPlayers() {
        ...
        if (videoPlayer) {
            videoPlayer.pause();
            videoPlayer.src = '';
            videoPlayer.load(); // ভিডিও রিমুভ করার জন্য
    ...
    // SwitchPlayer ফাংশনটি এখন শুধুমাত্র কী-বোর্ড শর্টকাট বা এরর হ্যান্ডেল করার জন্য
    ...
    // ... (populateGroupFilter এবং renderEpisodes কোড অপরিবর্তিত)
    populateGroupFilter() {
    ...
    playEpisode(index, isSwitching = false) {
        ...
        this.destroyAllPlayers(); // নতুন প্লে করার আগে সব প্লেয়ার বন্ধ
        this.updatePlayerVisibility(); // বর্তমান মোডের কন্টেইনার দেখানো
    ...
    playPlayerJS(episode, startTime) {
        ...
        this.playerjsInstance = new Playerjs({
            ...
            // PlayerJS এর জন্য প্রয়োজনীয় অন্যান্য সেটিংস...
        });
        
        this.startPositionSaver(this.playerjsInstance);
    },

    // ===== NEW CLAPPR PLAYER LOGIC =====
    playClappr(episode, startTime) {
        ...
        this.clapprInstance = new Clappr.Player({
            ...
            hlsjsConfig: {
                // HLS কনফিগারেশন
                startPosition: Math.floor(startTime)
            },
            autoPlay: true,
            width: '100%',
            height: '100%',
        ...
        // Clappr-এর জন্য পজিশন সেভার
        this.clapprInstance.on(Clappr.Events.PLAYER_PLAY, () => {
            this.clapprInstance.seek(startTime); // Start time ব্যবহার করা
    ...
    // ... (playEpisodeByUrl, nextEpisode, previousEpisode, saveProgress কোড অপরিবর্তিত)
    ...
    // ... (loadFromURL, loadFromText, loadFromFile, loadSavedPlaylist, saveCurrentPlaylist কোড অপরিবর্তিত)
...
// ===== Initialize App (Added initial player visibility) =====
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Default Load
    if (Router.currentPage === 'home' || Router.currentPage === 'player') {
