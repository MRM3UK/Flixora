...
const Player = {
    ...
    async loadFromURL(url, name) {
        ...
        try {
            ...
            this.loadPlaylist(episodes, name || 'Playlist');
        } catch (error) {
    ...
    loadFromText(content, name) {
        ...
        const episodes = M3UParser.parse(content);
        this.loadPlaylist(episodes, name || 'Pasted Playlist');
    },
    ...
    loadFromFile(file) {
        ...
        reader.onload = (e) => {
            ...
            const episodes = M3UParser.parse(content);
            this.loadPlaylist(episodes, file.name.replace(/\.[^/.]+$/, ""));
        };
    ...
    savePlaylist(name) {
        if (!name || this.currentPlaylist.length === 0) {
            Utils.showToast('Please enter a name and load a playlist first', 'error');
            return;
        }
    ...
    loadSavedPlaylist(name) {
        const playlists = Utils.getStorage(CONFIG.STORAGE_KEYS.PLAYLISTS) || {};
        if (playlists[name]) {
            this.loadPlaylist(playlists[name].episodes, name);
        }
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        Player.loadFromURL(CONFIG.DEFAULT_PLAYLIST_URL, CONFIG.DEFAULT_PLAYLIST_NAME);
    }, 500);
});
