...
// ===== Statistics Dashboard (No change) =====
const Stats = {
    render() {
        const playlists = Utils.getStorage(CONFIG.STORAGE_KEYS.PLAYLISTS) || {};
        const history = WatchHistory.getHistory();
...
// ===== Player (Core) - FIXED Error Handling and Initialization =====
const Player = {
    currentPlaylist: [],
    currentIndex: -1,
    hlsInstance: null,
    ...
    init() {
        ...
        if (videoPlayer) {
            ...
            // CRITICAL FIX: Ensure handler is bound once
            if (!this.handleErrorBound) {
    ...
    destroyAllPlayers() {
        ...
        if (videoPlayer) {
            ...
            videoPlayer.src = '';
            videoPlayer.load(); 
            // NOTE: We keep the error listener bound to this.handleErrorBound to avoid adding multiple listeners
        }
        if (this.hlsInstance) {
    ...
    playNative(episode, startTime, isSwitching = false) {
        ...
        // CRITICAL FIX: Ensure handler is removed/re-added to avoid duplicate events
    ...
    playClappr(episode, startTime) {
        ...
        this.clapprInstance.on(Clappr.Events.PLAYER_PLAY, () => {
             // FIXED: Only seek if there's a resume position
             if (startTime > 0 && this.clapprInstance.getCurrentTime() < 5) {
    ...
    async loadFromURL(url, name, isDefault = false) {
        ...
        try {
            ...
            // CORS error handling is a common reason for failure here in local environment
            Utils.showToast('Failed to load playlist from URL. This might be a CORS issue if running locally.', 'error');
