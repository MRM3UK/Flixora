...
const Player = {
    ...
    init() {
        ...
        if (videoPlayer) {
            ...
            // প্লেয়ার ও সেভ বাটন ইভেন্ট লিসেনার যোগ করা
            videoPlayer.addEventListener('error', this.handlePlaybackError.bind(this));
        }
        
        const switchBtn = document.getElementById('player-switch-btn');
        if (switchBtn) {
            // *** নতুন: Switch বাটন ক্লিক হ্যান্ডলার যোগ করা ***
    ...
    // *** SwitchPlayer ফাংশনটি ঠিক করা হয়েছে ***
    switchPlayer() {
        ...
        if (oldMode === 'native') {
            ...
            Utils.showToast('Switched to Player Two (PlayerJS) 🎬', 'info');
        } else {
            this.currentPlayerMode = 'native';
            localStorage.setItem(CONFIG.STORAGE_KEYS.PLAYER_MODE, 'native');
            Utils.showToast('Switched to Player One (Native) 📺', 'info');
        }
        
        // বর্তমান এপিসোড যদি প্লে হচ্ছিল, তবে নতুন মোডে আবার প্লে করা
    ...
    // *** playEpisode ফাংশন পরিবর্তন: অটো-সুইচ কন্ট্রোলের জন্য প্যারামিটার যোগ করা হয়েছে ***
    playEpisode(index, isSwitching = false) {
        ...
        if (this.currentPlayerMode === 'native') {
            // isSwitching প্যারামিটার পাস করা হয়েছে
    ...
    // *** playEpisode ফাংশন পরিবর্তন শেষ ***
    
    // *** handlePlaybackError ফাংশন যোগ করা হয়েছে: প্লেব্যাক এরর হ্যান্ডেল করার জন্য ***
    handlePlaybackError(event) {
        // প্লেব্যাক এরর হলে অটোমেটিক অন্য প্লেয়ারে সুইচ করার লজিক
        if (this.currentPlayerMode === 'native' && !event.target.seeking) {
            Utils.showToast('Native player failed. Switching to Player Two.', 'error');
            this.switchPlayer();
        }
    },
    // *** handlePlaybackError ফাংশন শেষ ***

    // *** playNative ফাংশন পরিবর্তন: এরর লিসেনার যোগ করা এবং অটো-সুইচ কন্ট্রোল ***
    playNative(episode, startTime, isSwitching = false) {
        ...
        // এরর হ্যান্ডলারকে একবারের জন্য সরিয়ে আবার যোগ করা, যাতে বারবার ট্রিগার না হয়
        videoPlayer.removeEventListener('error', this.handlePlaybackError.bind(this));
        if (!isSwitching) {
            // যদি এটি প্লেয়ার সুইচ করার ফলে প্লে না হয়, তবেই এরর লিসেনার যোগ করা হবে
            videoPlayer.addEventListener('error', this.handlePlaybackError.bind(this));
        }
        ...
        if (episode.url.includes('.m3u8')) {
            if (typeof Hls !== 'undefined' && Hls.isSupported()) {
                ...
                this.hlsInstance.on(Hls.Events.MANIFEST_PARSED, () => {
                    ...
                    videoPlayer.play().catch(e => {
                        // যদি অটো-প্লে ব্যর্থ হয়, তবে ম্যানুয়ালি সুইচ করার সুযোগ দেওয়া 
                        if (e.name === 'NotAllowedError' && !isSwitching) {
                            Utils.showToast('Autoplay blocked. Tap to play or try Player Two.', 'warning');
                        } else if (!isSwitching) {
                             // অটো-প্লে ব্যর্থ হলে এবং এরর না হলে, এরর লিসেনার অপেক্ষা করবে
                        }
                    });
                });
            ...
            videoPlayer.currentTime = startTime;
            videoPlayer.play().catch(e => {
                // সরাসরি URL প্লেতে সমস্যা হলে (যদি এরর ইভেন্ট ফায়ার না হয়)
    ...
    playPlayerJS(episode, startTime) {
        ...
        const videoPlayer = document.getElementById('video-player');
        if (videoPlayer) videoPlayer.removeEventListener('error', this.handlePlaybackError.bind(this));
        ...
        // PlayerJS এ এরর হ্যান্ডলিং সরাসরি সম্ভব নয়, তাই সেভার লজিক চালানো হচ্ছে
